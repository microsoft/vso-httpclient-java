...
public abstract class AlmHttpClientBase {
    ...
    protected AlmHttpClientBase(final Object tfsConnection) {
        this.rsClient = null;
        this.baseUrl = null;
    ...
    protected AlmHttpClientBase(final Object rsClient, final URI baseUrl) {
    ...
    protected <TEntity> Invocation createRequest(final HttpMethod method,
        ...
        if (shouldOverrideHttpMethod(method)) {
            ...
            requestBuilder.header(AlmHttpHeaders.HTTP_METHOD_OVERRIDE, method);
        } else {
            httpMethod = method;
        }

        requestBuilder.header(AlmHttpHeaders.TFS_VERSION, clientProperties.getProperty("version"));
    ...
    protected void handleResponse(final Response response) {
        if (response.getStatusInfo() == Response.Status.PROXY_AUTHENTICATION_REQUIRED) {
            ...
            if (exceptionToThrow == null || !(exceptionToThrow instanceof VssException)) {
                ...
                if (headers.containsKey(AlmHttpHeaders.TFS_SERVICE_ERROR)) {
                    try {
                        message = URLDecoder.decode(headers.getFirst(AlmHttpHeaders.TFS_SERVICE_ERROR), "UTF-8"); //$NON-NLS-1$
    ...
    private static void loadClientProperties()
    {
        try
        {
            final InputStream in = AlmHttpClientBase.class.getResourceAsStream("client.properties");
